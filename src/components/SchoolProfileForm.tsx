...
export default function SchoolProfileForm({ 
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const isFieldLocked = isIdentityLocked 
      ...
      : isLocked 
        ? (schoolInfo.teacherTitle === "Guru Kelas"
    ...
    <div id="school-profile-form-container" className="bingkai-emas-premium overflow-hidden p-4 md:p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-3.5 bg-slate-900/35 p-4 rounded-xl border border-slate-800/80 hover:border-amber-500/10 transition-all duration-350 shadow-inner">
          ...
          {(() => {
            const isClassSelectLocked = isIdentityLocked || (isLocked && schoolInfo.teacherTitle === "Guru Kelas");
          ...
          <div className="space-y-4 pt-3 border-t border-slate-850">
            <h4 className="text-[10px] font-extrabold text-amber-500 uppercase tracking-widest block">
              Pengaturan Logo Kop Surat (2 Logo)
            </h4>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  {schoolInfo.logoCustomData ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Logo Kabupaten</span>
                      <span className="text-[8px] text-slate-500 mt-0.5">Tut Wuri (Default) / Kustom</span>
                    </div>
                  )}
                </div>
                ...
                <div className="p-2.5 bg-slate-950/70 border border-slate-850 rounded-lg space-y-1.5 flex flex-col justify-center">
                  <span className="text-[9px] text-[#93c5fd]/75 font-black uppercase tracking-wider block">Unggah Logo Kabupaten</span>
                  <div className="flex items-center bg-slate-900 border border-slate-800 rounded-md p-1">
                    <input
                      type="file"
                      id="logo-kabupaten-file-picker"
                      accept="image/*"
                      onChange={(e) => {
                      ...
                      className="block w-full text-[9px] text-slate-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-[9px] file:font-black file:uppercase file:bg-amber-500/10 file:text-amber-400 hover:file:bg-amber-500/20 cursor-pointer"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  {schoolInfo.logoSchoolCustomData ? (
                    <div className="relative group/logo">
                      <img
                        src={schoolInfo.logoSchoolCustomData}
                        alt="Logo Sekolah"
                        className="h-14 object-contain mx-auto rounded border border-slate-800 p-0.5 bg-slate-900 shadow-sm transition-transform duration-300 group-hover/logo:scale-105"
                        referrerPolicy="no-referrer"
                      />
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Logo Sekolah</span>
                      <span className="text-[8px] text-slate-500 mt-0.5">Belum Diunggah (Kosong)</span>
                    </div>
                  )}
                </div>

                {/* Custom School Logo Upload Input Panel */}
                <div className="p-2.5 bg-slate-950/70 border border-slate-850 rounded-lg space-y-1.5 flex flex-col justify-center">
                  <span className="text-[9px] text-[#93c5fd]/75 font-black uppercase tracking-wider block">Unggah Logo Sekolah</span>
                  <div className="flex items-center bg-slate-900 border border-slate-800 rounded-md p-1">
                    <input
                      type="file"
                      id="logo-sekolah-file-picker"
                      accept="image/*"
                      onChange={(e) => {
                        const file = e.target.files?.[0];
                        if (file) {
                          ...
                          reader.onload = (event) => {
                            onChange({
                              ...schoolInfo,
                              logoSchoolCustomData: event.target?.result as string,
                            });
                          };
                          reader.readAsDataURL(file);
                        }
