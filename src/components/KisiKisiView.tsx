...
export default function KisiKisiView({
  ...
  const handleDownloadWord = async () => {
    ...
    // Agar logo dan gambar muncul di semua versi Word/Windows tanpa bergantung internet
    const convertImagesToBase64 = async (htmlString: string): Promise<string> => {
      ...
      for (const match of matches) {
        ...
        if (srcUrl.startsWith("data:") || srcUrl.startsWith("blob:")) continue;
        try {
          const response = await fetch(srcUrl, { mode: "cors", cache: "force-cache" });
          ...
          result = result.replace(fullTag, `${prefix}${base64}${suffix}`);
        } catch (err) {
          console.warn(`[Word Export KisiKisi] Gagal konversi: ${srcUrl}`, err);
        }
      }
      return result;
    };
    ...
    // Clone DOM agar tidak merusak tampilan asli di layar
    ...
    try {
      ...
      console.warn("[Word Export KisiKisi] Konversi base64 gagal, melanjutkan:", err);
    }
    ...
    <div id="kisi-kisi-view-root" className="space-y-6">
      ...
      <div className="bingkai-emas-premium p-6 flex flex-col lg:flex-row items-center justify-between gap-5 relative overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-end relative z-10">
          ...
          {!hasGeneratedSoal && (
            <button
              id="btn-trigger-soal-generation"
              type="button"
              onClick={onGenerateSoal}
              disabled={isGeneratingSoal}
              className="px-5 py-3.5 bingkai-emas-button rounded-xl inline-flex items-center gap-2 transition-all duration-300 cursor-pointer disabled:opacity-50 animate-pulse"
            >
              <span className="text-sm">⚡</span>
              {isGeneratingSoal ? "Menghasilkan Naskah Soal..." : "Generate Naskah Soal"}
            </button>
          )}
        </div>
      </div>
      ...
      <div className={isFullScreen 
        ? "fixed inset-0 z-[100] bg-slate-100 overflow-y-auto p-4 md:p-8 flex flex-col items-center" 
        : "bingkai-emas-premium overflow-hidden p-6 md:p-8 overflow-x-auto"
      }>
        {isFullScreen && (
