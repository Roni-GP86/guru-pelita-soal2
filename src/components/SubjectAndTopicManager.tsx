...
export default function SubjectAndTopicManager({
  ...
  const isGuruKelas = isLocked && schoolInfo?.teacherTitle === "Guru Kelas";
  const isGuruMapel = isLocked && schoolInfo?.teacherTitle === "Guru Mata Pelajaran";
  ...
  const fetchCurriculum = async (subjectInput: SubjectType) => {
    ...
    try {
      ...
      const isFallback = response.headers.get("X-Fallback-Used") === "true";
      if (onFallbackChange) {
        onFallbackChange(isFallback);
      }
      localStorage.setItem("ttu_fallback_active", isFallback ? "true" : "false");
