...
export default function App() {
  ...
  const [adminKey1, setAdminKey1] = useState(() => localStorage.getItem("ttu_system_key_1") || "");
  const [adminKey2, setAdminKey2] = useState(() => localStorage.getItem("ttu_system_key_2") || "");
  const [adminKey3, setAdminKey3] = useState(() => localStorage.getItem("ttu_system_key_3") || "");

  const handleSaveAdminKeys = async (k1: string, k2: string, k3: string) => {
    const cleanK1 = k1.trim();
    const cleanK2 = k2.trim();
    const cleanK3 = k3.trim();
    setAdminKey1(cleanK1);
    setAdminKey2(cleanK2);
    setAdminKey3(cleanK3);
    localStorage.setItem("ttu_system_key_1", cleanK1);
    localStorage.setItem("ttu_system_key_2", cleanK2);
    localStorage.setItem("ttu_system_key_3", cleanK3);

    try {
      await setDoc(doc(db, "systemSettings", "geminiKeys"), {
        key1: cleanK1,
        key2: cleanK2,
        key3: cleanK3,
        updatedAt: new Date().toLocaleString("id-ID")
      });
      setSuccessToast("🔑 Konfigurasi API Key Sistem berhasil diperbarui di Firestore & lokal!");
    } catch (err: any) {
      console.error("Gagal menyimpan API Key ke Firestore:", err);
      setWarningToast("⚠️ Gagal menyimpan ke Firestore. Tetapi perbaikan telah disimpan secara lokal.");
    }
  };
  ...
  const isCodeActive = isAdmin || (!!matchedRequest && matchedRequest.isActive !== false && matchedRequest.uniqueCode.trim().toUpperCase() !== "GP-RN00R");
  ...
  const effectiveSelectedSubject = isAdmin 
    ? selectedSubject 
    : (matchedRequest 
        ? (matchedRequest.role === "Guru Kelas"
  ...
  // Listen for admin API keys in systemSettings
  useEffect(() => {
    const unsubscribeKeys = onSnapshot(
      doc(db, "systemSettings", "geminiKeys"),
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          setAdminKey2(k2);
          setAdminKey3(k3);
          localStorage.setItem("ttu_system_key_1", k1);
          localStorage.setItem("ttu_system_key_2", k2);
          localStorage.setItem("ttu_system_key_3", k3);
        }
      },
      (err) => {
        console.warn("Could not listen to systemSettings/geminiKeys (permissions/not created yet):", err);
      }
    );
  ...
  const handleGenerateKisiKisi = async () => {
    ...
    try {
      const data = await apiGenerateKisiKisi(
        effectiveSchoolInfo,
  ...
  const handleGenerateSoalUjian = async () => {
    ...
    try {
      const data = await apiGenerateSoal(
        effectiveSchoolInfo,
        effectiveSelectedSubject,
        kisiKisi
      );
  ...
  const stepsList = [
    ...
    { 
      ...
      activeBg: "bg-gradient-to-r from-purple-600 to-blue-500 text-white shadow-lg shadow-purple-500/20",
      activeTextColor: "text-purple-650",
      borderActive: "border-purple-500",
      badgeColor: "bg-purple-100 text-purple-700"
    },
  ].filter(step => step.num !== 6 || isAdmin);
  ...
  return (
    <div id="app-root-container" className="min-h-screen comel-dot-pattern text-slate-300 flex flex-col font-sans selection:bg-blue-600/30 selection:text-white antialiased relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:flex-row w-full relative z-10 min-h-0">
        ...
        <main className="flex-1 px-5 py-5 md:px-6 md:py-6 space-y-5 min-w-0 bg-slate-950/20 relative z-10">
          ...
          <PremiumLoader
            ...
            title="AI Sedang Menyusun Kisi-Kisi Ujian"
          ...
          <PremiumLoader
            ...
            title="AI Sedang Menulis Naskah Soal Lengkap"
            ...
            <div id="workspace-step-render" className="animate-in fade-in duration-300">
              ...
              {activeStep === 6 && isAdmin && (
                <div id="step-6-menu-kode" className="space-y-6">
                  ...
                  {/* DYNAMIC API KEY CONFIGURATION FORM */}
                  <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 md:p-6 space-y-4">
                    <div className="border-b border-slate-800 pb-3">
                      <h3 className="font-black text-slate-200 text-sm flex items-center gap-2">
                        <span>🔑</span> KONFIGURASI API KEY GEMINI (ROTATING POOL)
                      </h3>
                      <p className="text-[11px] text-slate-500 mt-1">
                        Masukkan hingga 3 API Key Gemini utama. Sistem akan otomatis merotasi kunci-kunci ini untuk menghindari limit kuota (rate limiting) secara realtime tanpa perlu mendeploy ulang kode program. Kunci disimpan terenkripsi di database Firestore Anda.
                      </p>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-1 text-left">
                        <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider block">API Key Utama (Key 1)</label>
                        <input
                          type="password"
                          value={adminKey1}
                          onChange={(e) => setAdminKey1(e.target.value)}
                          placeholder="Masukkan API Key 1..."
                          className="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-lg text-xs font-mono text-slate-300 focus:border-amber-500/50 focus:outline-none"
                        />
                      </div>
                      <div className="space-y-1 text-left">
                        <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider block">API Key Cadangan (Key 2)</label>
                        <input
                          type="password"
                          value={adminKey2}
                          onChange={(e) => setAdminKey2(e.target.value)}
                          placeholder="Masukkan API Key 2..."
                          className="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-lg text-xs font-mono text-slate-300 focus:border-amber-500/50 focus:outline-none"
                        />
                      </div>
                      <div className="space-y-1 text-left">
                        <label className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wider block">API Key Cadangan (Key 3)</label>
                        <input
                          type="password"
                          value={adminKey3}
                          onChange={(e) => setAdminKey3(e.target.value)}
                          placeholder="Masukkan API Key 3..."
                          className="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-lg text-xs font-mono text-slate-300 focus:border-amber-500/50 focus:outline-none"
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                      <button
                        type="button"
                        onClick={() => handleSaveAdminKeys(adminKey1, adminKey2, adminKey3)}
                        className="px-4 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs font-black rounded-lg uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-md transition-all active:scale-95"
                      >
                        <Key size={13} />
                              ...
                              <div className="mt-4 md:mt-0 flex flex-col sm:flex-row md:flex-col gap-3 bg-slate-950/80 border border-slate-850 rounded-2xl p-4 shrink-0 min-w-[180px] text-center md:text-left shadow-inner">
                                ...
                                <div className="flex flex-col gap-1.5">
                                  ...
                                  <button
                                    type="button"
                                    onClick={async () => {
                                      if (req.uniqueCode.trim().toUpperCase() === "GP-RN00R") {
                                        alert("Kode GP-RN00R adalah contoh simulasi dan tidak dapat diaktifkan!");
                                        return;
                                      }
      ...
      {successToast && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 border border-emerald-500/45 text-emerald-400 font-extrabold text-[11px] px-4 py-3 rounded-xl shadow-2xl flex items-center gap-2 animate-in slide-in-from-bottom duration-300">
      ...
      {warningToast && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 border border-amber-500/45 text-amber-400 font-extrabold text-[11px] px-4 py-3 rounded-xl shadow-2xl flex items-center gap-2 animate-in slide-in-from-bottom duration-300 max-w-sm">
          ...
          <button 
            ...
            className="ml-2 hover:text-white shrink-0"
          >
            ✕
          </button>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form 
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  setSuccessToast("Bukti transfer berhasil terkirim! Silakan buka kembali aplikasi Pelita Soal secara berkala untuk mengecek keaktifan kode akses Anda (Admin tidak wajib membalas via WhatsApp).");

                  // Background update to Firestore without blocking UI
                  setDoc(doc(db, "codeRequests", finalRecord.id), finalRecord).catch((err) => {
                    console.error("Background Firestore sync failed in message submit:", err);
                  });
                } catch (err) {
                  console.error("Gagal memproses bukti transfer:", err);
                  alert("Gagal memproses bukti transfer.");
                }
              }} 
            ...
            >
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  type="text"
                  placeholder="Contoh: GP-RN00R (Kosongkan jika ingin dibuatkan baru)"
              ...
              <div className="pt-4 flex items-center justify-end gap-3.5 border-t border-slate-850">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-500 hover:from-blue-500 hover:to-indigo-400 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-blue-500/10 cursor-pointer transition-transform active:scale-95"
                >
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    // 1b. Automatically pre-fill the Message & Proof Form with these exact requested details!
                    setMessageForm({
                    ...
                    setSuccessToast(`Pengajuan Berhasil! Data direkam. Informasikan pada Admin untuk mengaktifkan kode: ${code}`);

                    // 5. Fire background call to Firestore without blocking the UI
                    setDoc(doc(db, "codeRequests", newRecord.id), newRecord).catch((err) => {
                      console.error("Background Firestore sync failed for code request:", err);
                    });
                  } catch (err) {
                ...
                className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-500 hover:from-emerald-500 hover:to-green-400 text-slate-950 text-xs font-black uppercase tracking-wider rounded-xl cursor-pointer"
              >
                Ya, Kirim Data!
              </button>
            </div>
