...
/**
 * Direct Imagen model generation function provided by the user.
 * Prioritizes client-side API Key pool rotation, falling back to server-side if none is active.
 */
export async function generateImage(imagePromptText: string): Promise<string | null> {
  const keysPool = getApiKeysPool();

  const enhancedPrompt = `Create an educational illustration for Indonesian elementary school (SD) students. 
${imagePromptText}
Style: Clean 2D vector flat illustration, white background, educational clipart style, no text, no letters, suitable for exam paper.`;
  ...
  // Helper: Panggil model Gemini yang support generate gambar (responseModalities IMAGE)
  async function tryGeminiImageModel(modelId: string, activeKey: string): Promise<string | null> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.warn(`[imageUtils] ${modelId} error ${res.status}:`, err?.error?.message || "");
        return null;
      }
      ...
      if (json.candidates?.[0]?.content?.parts) {
        for (const part of json.candidates[0].content.parts) {
          if (part.inlineData?.data) {
            ...
            console.log(`[imageUtils] Gambar berhasil dibuat via ${modelId}`);
  ...
  // Helper: Panggil Imagen via REST API (butuh billing GCP)
  async function tryImagenModel(modelId: string, activeKey: string): Promise<string | null> {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:predict?key=${activeKey}`;
    const payload = {
  ...
  for (const activeKey of keysPool) {
    ...
    // ===== URUTAN MODEL: TERBARU → TERLAMA =====
    // Model 1: Gemini 3.1 Flash Image Preview (terbaru)
    let result = await tryGeminiImageModel("gemini-3.1-flash-image-preview", activeKey);
    if (result) return result;

    // Model 2: Gemini 2.5 Flash Preview (dengan kemampuan image generation)
    result = await tryGeminiImageModel("gemini-2.5-flash-preview-05-20", activeKey);
    if (result) return result;

    // Model 3: Gemini 2.0 Flash Image Generation (resmi didokumentasikan untuk generate gambar)
    result = await tryGeminiImageModel("gemini-2.0-flash-preview-image-generation", activeKey);
    if (result) return result;

    // Model 4: Imagen 3.0 Generate 002 (jika user punya akses billing GCP)
    result = await tryImagenModel("imagen-3.0-generate-002", activeKey);
    if (result) return result;

    // Model 5: Imagen 3.0 Generate 001 (legacy fallback)
    result = await tryImagenModel("imagen-3.0-generate-001", activeKey);
    if (result) return result;
  }

  console.warn("[imageUtils] Semua model dan semua API Key client-side gagal. Fallback ke server proxy.");
  return null;
}
...
export async function generateImageForSoal(
  ...
  if (!prompt) {
    ...
    prompt = `Vibrant 2D vector flat educational illustration ${materiContext} for elementary school grade exam. Subject description or visual context: "${question.questionText}". Focus purely on the main visual elements. Absolutely white clean background, neat educational clipboard clipart style. Clean flat lines. ABSOLUTELY NO letters, labels, or written text.`;
  }
  ...
  try {
    ...
    console.log(`[imageUtils] API key client tidak tersedia. Mencoba backend /api/generate-image...`);
    const userApiKey = localStorage.getItem("ttu_user_api_key") || "";
